...
const { validateBatch, answerLetterToIndex, normalizeText } = require('./lib/exercise-validation');
...
/**
 * Das Textformat aus exercise-prompt.js einlesen.
 *
 * Der Aufgabenteil und der Lösungsschlüssel stehen getrennt. Das ist Absicht:
 * Das Modell muss jede Aufgabe erst lösen und die Lösung danach ausgeschrieben
 * wiederholen — Buchstabe UND Text. Stimmen beide nicht überein, war die Lösung
 * geraten, und die Aufgabe fliegt raus.
 */
function parseTextExercises(rawText, expectedCount) {
  ...
  while ((keyMatch = keyRegex.exec(keysText))) {
    ...
    if (number > 0 && index >= 0) {
      keyMap.set(number, { index, answerText: keyMatch[3] ? keyMatch[3].trim() : '' });
    }
  }
  ...
  for (const block of blocks) {
    ...
    const key = keyMap.get(Number(numberMatch[1]));
    if (!key) continue;
    ...
    for (const line of lines) {
      ...
      if (optionMatch) {
        optionLines.push({ label: optionMatch[1].toUpperCase(), value: optionMatch[2].trim() });
    ...
    const options = ['A', 'B', 'C', 'D'].map(
      (label) => optionLines.find((option) => option.label === label)?.value || ''
    );
    if (options.some((option) => !option)) continue;

    const pick = (pattern) => bodyLines.find((line) => pattern.test(line));
    const formatLine = pick(/^Format\s*:/i);
    const instructionLine = pick(/^Anweisung\s*:/i);
    const displayLine = pick(/^(Aufgabe|Satz|Wörter|Woerter|Dialog)\s*:/i);

    const display = displayLine
      ? displayLine.replace(/^(Aufgabe|Satz|Wörter|Woerter|Dialog)\s*:\s*/i, '').trim()
      : bodyLines.find((line) => !/^(Format|Anweisung)\s*:/i.test(line));

    parsed.push({
      format: formatLine ? formatLine.replace(/^Format\s*:\s*/i, '').trim().toLowerCase() : null,
      ...
      options,
      correct: key.index,
      correctAnswer: key.answerText || options[key.index] || '',
    });
...
function installQuizRoutes(app) {
  ...
  app.post('/api/generate-questions', async (req, res) => {
    ...
    // Etwas mehr anfordern als gebraucht: Substanz- und Dublettenprüfung
    // sortieren regelmäßig einzelne Aufgaben aus.
    ...
    try {
      ...
      const parsed = parseTextExercises(text, requestCount);
      ...
      const label = `${resolveTopic(grammarTopic) || grammarTopic} ${level}`;
      if (rejected.length) {
        console.warn(`[${label}] ${questions.length}/${parsed.length} Aufgaben übernommen; aussortiert: ${rejected.join('; ')}`);
      }
      if (questions.length) {
        console.log(`[${label}] Formate:`, formatCounts);
      }

      if (!questions.length) {
        return res.status(502).json({
          error: 'No valid questions in LLM response',
          detail: rejected.slice(0, 5).join('; ') || 'Antwort ließ sich nicht parsen',
        });
      }
...
module.exports = { installQuizRoutes, parseTextExercises };
