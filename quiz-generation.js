...
const TOPIC_RULES = {
  ...
  'Modalverben in Präteritum': `Modalverben im Präteritum: können→konnte/konntest/konnte/konnten/konntet/konnten, müssen→musste/musstest/musste/mussten/musstet/mussten, dürfen→durfte/durftest/durfte/durften/durftet/durften, wollen→wollte/wolltest/wollte/wollten/wolltet/wollten, sollen→sollte/solltest/sollte/sollten/solltet/sollten, mögen→mochte/mochtest/mochte/mochten/mochtet/mochten. Kein Umlaut in den Präteritumformen: konnte, musste, durfte, mochte. Modalverb steht auf Position 2, Vollverb als Infinitiv am Satzende ohne "zu". Richtig: "Ich konnte nicht schwimmen." | Falsch: "Ich könnte nicht schwimmen.", "Ich konnte nicht zu schwimmen.", "Ich durfte gehen nicht."`,
...
function isModalPreteritumTopic(grammarTopic) {
  const target = normalizeTopicKey(grammarTopic);
  return target === 'modalverbeninprateritum' || target === 'modalverbeninpraeteritum';
}

function modalPreteritumSystemPrompt(grammarTopic) {
  if (!isModalPreteritumTopic(grammarTopic)) return '';

  return `Du erstellst ausschliesslich Aufgaben zum Thema "Modalverben in Praeteritum".
Die Zielkompetenz ist: deutsche Modalverben vom Praesens ins Praeteritum setzen und die Satzklammer korrekt bauen.
Erlaubte Zielformen: konnte, konntest, konnten, konntet; musste, musstest, mussten, musstet; durfte, durftest, durften, durftet; wollte, wolltest, wollten, wolltet; sollte, solltest, sollten, solltet; mochte, mochtest, mochten, mochtet.
Baue gezielte Ablenker mit unnoetigem Umlaut (koennte/muesste/duerfte), falscher Person oder falschem Numerus, Praesens statt Praeteritum, falscher Satzstellung oder falschem "zu" vor dem Infinitiv.
Nutze gemischt genau diese Aufgabenarten:
1. Einsetzen mit Hinweis: Luecke plus Infinitiv in Klammern, z.B. "Als ich klein war, ___ ich nicht schwimmen. (koennen)".
2. Richtige Variante waehlen: eine Luecke und drei oder vier kurze Optionen.
3. Praesens ins Praeteritum: ein Praesenssatz und vier ausgeschriebene Satzvarianten.
4. Satz bauen: Woerter durcheinander und vier komplette Satzvarianten; Modalverb auf Position 2, Infinitiv am Ende.
5. Fehler finden: ein Satz mit genau einem Fehler in Form oder Wortstellung; Optionen nur "Ja" und "Nein".
Alle Aufgaben muessen natuerlich, eindeutig loesbar und fuer DaF-Lernende fair sein.`;
}

function modalPreteritumTaskRules(grammarTopic) {
  if (!isModalPreteritumTopic(grammarTopic)) return '';

  return `
Spezialregeln fuer Modalverben in Praeteritum:
- Verwende nur Modalverben im Praeteritum: konnte, musste, durfte, wollte, sollte, mochte und die passenden Personen/Numeri.
- Verwende keine Konjunktiv-II-Formen als richtige Antwort: koennte, muesste, duerfte sind Ablenker, nicht Loesungen.
- Vollverb steht als Infinitiv am Satzende und NIE mit "zu": "Ich konnte nicht schwimmen.", nicht "Ich konnte nicht zu schwimmen."
- Mische diese Aufgabenarten gleichmaessig:
  1. Einsetzen mit Hinweis: Satz mit ___ und Infinitiv in Klammern; 4 Optionen.
  2. Richtige Variante waehlen: eine Luecke; 3 oder 4 Optionen.
  3. Praesens ins Praeteritum: Praesenssatz als Aufgabe; 4 ausgeschriebene Varianten.
  4. Satz bauen: Woerter/Phrasen durcheinander; 4 komplette Satzvarianten.
  5. Fehler finden: ein Satz mit genau einem Fehler; nur 2 Optionen (Ja/Nein).
- Gute Ablenker: unnoetiger Umlaut (konnte/koennte, musste/muesste, durfte/duerfte), falsches Subjekt-Matching (ich konnte vs. ich konnten), Praesens statt Praeteritum, falsche Satzklammer, falsches "zu".
- Wenn das lexikalische Thema "Kindheit" ist, nutze Kontexte wie frueher, als ich klein war, Kindergarten, Schule, Freunde, Spielplatz, Schwimmen, Fahrrad fahren, Hausaufgaben.`;
}
...
function buildSyntheticPrompt({ level, lexicalTopic, grammarTopic, isWortstellung, questionsCount, exclude, topicRule }) {
  {
    ...
    const modalTaskBlock = modalPreteritumTaskRules(grammarTopic);
...
Uebungstyp: ${kind}
${ruleBlock}${modalTaskBlock}${excludeBlock}
Qualitaetsregeln:
...
function installQuizRoutes(app) {
  ...
  app.post('/api/generate-questions', async (req, res) => {
    ...
    try {
      const text = await requestAiText(prompt, 8192, modalPreteritumSystemPrompt(grammarTopic));
